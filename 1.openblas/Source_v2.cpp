...
void StoreData(float* list_pic_2d, int row, int col, int ch ) {
	...
	for (int k = 0; k < ch; k++ ) {
		...
		for (int i = 0; i < row; i++) {
			for (int j = 0; j < col; j++ ) {
				fprintf( fp, "%10.4f", list_pic_2d[j + i * col + k * row * col] );
			}
			fprintf(fp, "\n");
		}
		fprintf(fp, "\n");
...
void StoreMAE_style(float* list_pic_2d, int row, int col, int ch, string output_path ) {
...
void Create_CorrectXYZ( float* list_C, int row_C, int col_C, float* & extend )  {
 ...
 void Create_pic_spectrum(float* list_Spectrum, int row_Spectrum, int col_Spectrum, float* Extend2 ) {
	 /*----------------------------
	 ...
	 //list_pic_2d is final result but it is 3d matrix;
	 reshape_2d_to_3dtranspose(pic_rows, pic_cols, pic_channel, list_pic_2d);
...
void print_list_3D(float* list_3d, int row, int col, int ch ) {
 ...
 void Set_img_spectrum( string data_path, string filename ) {
	...
	Create_pic_spectrum(list_Spectrum, row_Spectrum, col_Spectrum, Extend2);
	free(Extend2);

	string output_path = "../Wetland/Algorithm_data/" + filename + ".txt";
	//StoreData(list_pic_2d, pic_rows, pic_cols, 401);
	StoreMAE_style(list_pic_2d, pic_rows, pic_cols, 401, output_path );
...
int main() {
	...
	for( int i = 0,index = 1; i < 10; i++, index++){
		...
		input_path = "../Wetland/" + int2str(index) + ".png";
		Set_img_spectrum( input_path, int2str(index));
