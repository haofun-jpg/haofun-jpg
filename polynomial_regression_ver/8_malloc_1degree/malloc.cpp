...
void load_image( unsigned short* R_list, unsigned short* G_list, unsigned short* B_list, int size, string filename ){
	fstream file;
...
int main(){
  ...
  printf("Time = %f\n", cpu_time_used);
   ...
   if( image_size < numthread ){
      ...
      std::thread myThread;
      myThread = std::thread(spectrum, 0, image_size, wave_size, parameter, ref(R_list), ref(G_list), ref(B_list), ref(w), ref(image_pixel));
      myThread.join();
   }
   else{
      //image_size >= numthread
      ...
      //Ex_Threads calc part*(numthread)-image_size
         ...
         Ex_Threads = std::thread(spectrum, part*numthread, image_size, wave_size, parameter, ref(R_list), ref(G_list), ref(B_list), ref(w), ref(image_pixel));
   ...
   //StoreCSV(image_pixel, row, col, wave_size, filename );
