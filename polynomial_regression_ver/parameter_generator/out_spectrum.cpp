...
int main(){
	...
	cv::Mat EV(401, 12, CV_32F);
	LoadData("EV.txt", EV, 401, 12, 0);
	cout << EV << endl;

	cv::Mat M(12, 7, CV_32F);
	LoadData("M.txt", M, 12, 7, 0);
	cout << M << endl;
	
	cv::Mat Spectrum(401, 7, CV_32F);
	Spectrum = EV*M;
	cout << Spectrum << endl;

	StoreData( fileName, Spectrum, 401, 7 );
