...
int StoreData( string fileName, cv::Mat& matData, int matRows = 0, int matCols = 0 ){
	...
	for( int i = 0; i < matRows; i++ ){
		for( int j = 0; j < matCols; j++ ){
			file << fixed  <<  setprecision(6) << matData.at<float>(i,j) << '\t';
		}
		file << endl;
	}
	file.close();
...
int main(){
	...
	cv::Mat White_light(3, 1, CV_32F);
	LoadData("light_white_point.txt", White_light, 3, 1, 0);
	cout << White_light << endl;

	cv::Mat White_D65(3, 1, CV_32F);
	LoadData("Camera_white_point.txt", White_D65, 3, 1, 0);
	cout << White_D65 << endl;
	
	cv::Mat XYZ(3, 3, CV_32F);
	cv::Mat diag_M = (Mat_Ma * White_light) / (Mat_Ma * White_D65);
	diag_M = diag_M.diag(diag_M);
	XYZ = Mat_Ma.inv() * diag_M * Mat_Ma;
	cout << XYZ << endl;

	StoreData( fileName, XYZ, 3, 3 );
